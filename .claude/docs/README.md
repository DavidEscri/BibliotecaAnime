...
|---|---|
| **Fecha** | 2026-09-02 |
| **Commit** | rama `feature/ui-redisign`, en `9083a32` (documentación). El rediseño está **entero commiteado** (fases 1-9, de `bd25742` a `7606def`), y también los cinco arreglos posteriores: el **fondo de la pantalla de carga** en `e7d8f2f`, el **hover de la lista de episodios** en `df47130`, el **refresco de la banda «Retomar»** en `4ffc2ef` y el **ancho de las fichas de género** en `1b63882`, y la **apertura de la ficha por un episodio** en `a0e3f37`, con su documentación intermedia en `f847c83`, `497128d`, `6b37bd2` y `a060ac8`. ⚠️ La rama **no** se ha fusionado con `main` |
| **Árbol de trabajo** | 🟡 **Con código pendiente de commitear**: las rejillas adaptables y la normalización de la sinopsis (**+321 líneas** en 7 módulos, ningún fichero nuevo, `.spec` sin tocar), más esta tanda de documentación ([02](02-mapa-de-modulos.md), [03](03-flujos-de-ejecucion.md), [06](06-gui-y-vistas.md), [07](07-concurrencia-e-hilos.md), [09](09-verificacion-y-pruebas.md), [10](10-invariantes-y-trampas.md), [12](12-deuda-tecnica-y-roadmap.md), `CLAUDE.md` y este índice). Sin trackear siguen los 4 PNG sin usar, `.vscode/`, `.claude/settings.local.json` y `.claude/plans/` (ver [12 §1](12-deuda-tecnica-y-roadmap.md)) |
| **Última revisión** | 2026-09-02 — **rejillas adaptables y sinopsis**: [10](10-invariantes-y-trampas.md) gana **dos trampas**, la **41** (una rejilla con `sticky="w"` mide su propio contenido, así que calcular columnas de su ancho es un punto fijo) y la **42** (`wraplength` no puede deshacer un `
` del texto), en un apartado propio —«Medir el sitio que tienes»— y con la **32** ampliada a los pesos `uniform` de `PosterGrid`; [09](09-verificacion-y-pruebas.md) estrena **§6f** (cómo comprobar que una rejilla se adapta, incluida la receta para manejar la app real desde PowerShell) y **§6g** (la normalización del texto, con `split()` como invariante); [06](06-gui-y-vistas.md) reescribe las fichas de `PosterGrid` y `Pager` y la tabla de vistas; [11](11-playbooks.md) añade la regla a §1 y §1b; [02](02-mapa-de-modulos.md) recuenta (**11 599 → 11 920**). Antes, 2026-09-01 — **abrir la ficha por un episodio**: [03](03-flujos-de-ejecucion.md) estrena el **flujo 12** (las tres acciones que prometen un episodio lo cumplen) y renumera el antiguo §12 a **§13**, con §3 y §8 al día; [10](10-invariantes-y-trampas.md) gana la trampa **40** —un `bind()` sin `add="+"` **borra** el manejador que el widget ya tenía, y excluir un botón de un recorrido no excluye su interior—, en un apartado propio; [09](09-verificacion-y-pruebas.md) estrena **§6e**, cómo pulsar de verdad un botón que vive dentro de otro widget, con los dos requisitos que hacen que la prueba no mienta; [06](06-gui-y-vistas.md) pone al día `AnimeRow`, la ficha y las dos vistas de cascada; [07](07-concurrencia-e-hilos.md) sube **C5** de gravedad; [02](02-mapa-de-modulos.md) recuenta (**11 430 → 11 599**). Antes, 2026-09-01 — **ancho de las fichas de género**: [10](10-invariantes-y-trampas.md) gana la trampa **39** —un `CTkButton` no respeta el `width` que se le pide, así que quien coloque con `place()` tiene que sumar lo que su rejilla interna reserva—, en un apartado propio y con la aritmética medida; [09](09-verificacion-y-pruebas.md) estrena **§6d**, cómo comprobar una medida de layout leyendo `winfo_reqwidth()` en vez de mirando la ventana; [06](06-gui-y-vistas.md) lo generaliza en `GenreChips` —en CustomTkinter `width` es una petición, no un contrato, y esa es la raíz común de las trampas **29**, **30** y **39**—; [02](02-mapa-de-modulos.md) recuenta (**11 403 → 11 430**). Antes, 2026-09-01 — **refresco de la banda «Retomar»**: [03](03-flujos-de-ejecucion.md) estrena el **flujo 11** (la portada relee los episodios de lo que estás viendo al entrar) y renumera el antiguo §11 a **§12**; [10](10-invariantes-y-trampas.md) gana la trampa **38** —una fila guardada no refresca sus episodios sola—, resuelta **solo** para las ≤3 tarjetas de la portada; [09](09-verificacion-y-pruebas.md) estrena **§6c**, cómo verificar en tres tramos algo que sale a la red, escribe en la biblioteca y repinta un widget; [07](07-concurrencia-e-hilos.md) **reancla §1, §2 y §6 enteras** —de los 13 hilos que listaba, **tres ya no existían** y solo cuatro líneas eran correctas—; [12](12-deuda-tecnica-y-roadmap.md) abre **B12** y cierra **B3** y **B8**. Antes, 2026-09-01 — **hover de la lista de episodios**: [10](10-invariantes-y-trampas.md) gana la trampa **37** —un hijo sin `bind` es una salida de la que no llega ningún `<Leave>`, la complementaria de la **34**—; [09](09-verificacion-y-pruebas.md) estrena **§6b**, cómo recorrer el hover con el puntero de píxel en píxel; [02](02-mapa-de-modulos.md) estrena la **ficha de `EpisodeRow`**, que no tenía; y [06](06-gui-y-vistas.md) explica por qué `AnimeRow` no sufre lo mismo. Antes, 2026-08-31 — **fondo de la pantalla de carga**: [10](10-invariantes-y-trampas.md) gana la trampa **36** (un widget sin `fg_color` sale del gris por defecto de CustomTkinter, no de `Theme`); el arranque se reancla en [03 §1](03-flujos-de-ejecucion.md), [06](06-gui-y-vistas.md), [07 §1-§4](07-concurrencia-e-hilos.md) y [02](02-mapa-de-modulos.md) —sus anclas de `main_window.py` iban ~29 líneas desplazadas—, y [09 §7.1](09-verificacion-y-pruebas.md) gana la comprobación del fondo. Antes, 2026-08-21 — **rediseño de interfaz (fases 1-9)**: [06](06-gui-y-vistas.md) **reescrito entero**, [09 §7](09-verificacion-y-pruebas.md) también; **7 trampas nuevas** (29-35) en [10](10-invariantes-y-trampas.md); fichas de `theme.py` y de los 11 componentes en [02](02-mapa-de-modulos.md); la columna `rating` en [04 §3b](04-modelo-de-datos.md); las tres preferencias nuevas en [13 §4](13-selector-de-proveedor.md); [11](11-playbooks.md) gana §1b y **corrige §5**, que mandaba activar unos iconos que no funcionan. Antes, 2026-08-18 — **sincronización de estado**: las cabeceras de los 14 documentos dejan de decir «árbol sucio» (la tanda de licencia entró en `e337d20`; la columna `provider_id`, en `a3d4331`) y se corrige el **árbol de directorios del `README.md`**, que cerraba **C10** |
| **Último cambio** | ✅ 2026-09-02 — **las rejillas se adaptan al ancho y la sinopsis envuelve**. Dos fallos con la misma forma: un widget que parecía adaptarse y nunca llegaba a mirar el sitio que tenía. Las cuatro vistas de rejilla iban colocadas con `sticky="w"` desde el rediseño, así que medían **su propio contenido**: maximizar dejaba casi 500 px de fondo muerto a la derecha con 6 columnas fijas. Ahora `PosterGrid` calcula las columnas del ancho (8 y 7 a 1920, los mismos 6 y 5 a 1440), reparte el sobrante con `weight` + `uniform`, y el tamaño de página pasa a ser `columnas × 2` para que la página siga siendo dos filas llenas. Y la sinopsis conservaba los renglones del proveedor por ancha que fuera la ventana: `wraplength` **no puede con un `
`**, así que se normaliza al pintar —saltos sueltos a espacio, párrafos conservados— y se quitó el tope de 74 caracteres. ✅ Verificado con 14 comprobaciones de geometría a cuatro tamaños, 12 casos de texto más las 35 sinopsis reales sin perder una palabra, y capturas de la aplicación real en las cuatro vistas y la ficha, a 1440, maximizada y a 900 px. Nacen las **trampas 41 y 42**. Antes, 2026-09-01 — **«Viendo» y «Pendientes» llevan al episodio, no al anime**. Sus tres acciones decían un episodio en su propio texto —«Episodio N →», «Seguir por el N» y «Empezar»— y las tres abrían la ficha arriba del todo. Ahora la abren con esa fila desplegada, sus servidores a la vista y la ventana desplazada **solo si hacía falta**; «Empezar» además fuerza el orden ascendente, y «Viendo» no —con un anime largo servido de mayor a menor, forzarlo dejaría el episodio fuera de los 25 que se pintan—. Si el episodio cae fuera de ese corte se enseña él solo, con sus botones de navegación, como al buscarlo a mano. 🔴 **Por el camino se destapó que la píldora de `AnimeRow` nunca había ejecutado su `command`**: la fila la excluía del clic, pero solo al botón y no a su canvas y su etiqueta, y `bind()` **sustituye** en vez de sumar, así que le estaba pisando el `_clicked` de CustomTkinter. Pulsar «Empezar» **no movía el anime a «Viendo»** desde el rediseño, en silencio. ✅ Verificado pulsando el interior de la píldora: antes corría el `on_click` de la fila; ahora, 10 casos de montaje de la ficha, 8 de ataduras y las 7 filas reales de la biblioteca (One Piece → 1164, Mushoku Tensei → 10, los 5 pendientes → 1 con orden forzado), con `DB_Animes.db` intacta. Nace la **trampa 40**. Antes, 2026-09-01 — **las fichas de género ya no se solapan al seleccionarlas**. `GenreChips` colocaba las fichas con `place()` llevando él la cuenta del ancho, y esa cuenta se quedaba corta: un `CTkButton` **no respeta el `width` que se le pide** —su rejilla interna propaga tamaño— y `place()` sin ancho pinta el widget a su ancho *pedido*. Faltaban 6 px por ficha, y **16 en cuanto se seleccionaba una**, porque estrena icono y negrita: se comían los 8 px de separación y la ficha siguiente le tapaba la ✕. Ahora `__chip_width()` suma también el radio de la píldora, los bordes de las dos etiquetas internas y el hueco texto↔icono. ✅ Medido leyendo los widgets con la raíz de Tk oculta: 4 estados de selección y los 41 chips desplegados en 4 anchos → **0 descuadres, 0 solapes**, hueco siempre 8 px, y alto pedido = alto real; ✅ confirmado además **en la aplicación** por el usuario. Nace la **trampa 39**. Antes, 2026-09-01 — **la banda «Retomar» ya no miente con los animes en emisión**. La columna `episodes` de una fila solo se reescribía al abrir su ficha, así que un anime que estrena los domingos decía «Lo has visto entero» con el capítulo nuevo ya publicado. Ahora, **al entrar en «Nuevos lanzamientos»** —no al arrancar—, un hilo daemon relee los episodios de las ≤3 filas de la banda y, si el proveedor sirve más, reescribe **solo** esa columna y repinta la tarjeta. Va con `strict=True`, sin *fallback*, porque escribe sin que el usuario lo pida. ✅ Verificado en tres tramos: la red real contra los 3 animes de la banda, la escritura sobre **copia** de la BD (9 → 10 episodios: «Lo has visto entero» → «Siguiente: episodio 10 de 10», con `watched_episodes` y estados intactos) y el repintado leyendo el widget con la raíz de Tk oculta. Nace la **trampa 38** y la deuda **B12**. Antes, 2026-09-01 — **el resaltado de la lista de episodios ya no se queda pegado**. `EpisodeRow` ataba el hover a la fila y a sus dos etiquetas, pero no al interruptor ni al separador de 1 px que hay entre episodios; como `CTkFrame.bind()` ata al canvas interno —**hermano** de esos hijos, no ancestro—, salir de la fila por uno de ellos no mandaba ningún `<Leave>` y el episodio se quedaba encendido. Recorrer la lista despacio dejaba **las cinco filas resaltadas a la vez**. Ahora el hover va a los cinco hijos, el clic sigue solo en tres, y un atributo de clase garantiza que **nunca haya dos**. ✅ Medido con `SetCursorPos` de píxel en píxel: antes 5 de 5, después 1 siempre y 0 al salir. Nace la **trampa 37**. Antes, 2026-08-31 — **el arranque tiene un solo fondo**. La pantalla de carga era un `CTkFrame` sin `fg_color`, así que salía del gris por defecto de CustomTkinter: el arranque enseñaba **tres grises a la vez** y el GIF, que es transparente, heredaba el del marco. Ahora cubre la ventana entera con `Theme.BG`, se centra con `place` relativo (antes multiplicaba `winfo_width()` por 2.5, que vale 1 antes de dibujar) y el texto y la barra salen de `Theme`. ✅ Medido con captura y muestreo de píxeles en los dos temas. Nace la **trampa 36**. Antes, 2026-08-21 — **la interfaz está rediseñada de arriba abajo**: `gui/theme.py` y **11 componentes compartidos** nuevos, barra lateral plegable y persistida, las 6 vistas y la ficha rehechas, calificación personal en favoritos (columna `rating`) y estados vacíos en las 7 vistas. **+5 064 líneas**. Verificado con la app real en los dos temas y el `.exe` compilado y arrancado. Cierra el TODO de `main_window.py:32` y **5 puntos del roadmap**. Antes, 2026-08-17 — **el proyecto es formalmente GPL-3.0-or-later**: `LICENSE`, aviso de copyright, `LEEME.txt` y `THIRD-PARTY-NOTICES.txt` junto al `.exe`. El `.spec` deja de empaquetar la BD y los pósters del desarrollador y queda **verificado compilando y arrancando el `.exe`** por primera vez. Cierra **A3**, **C11** y **C10**. Publicado como **`v0.2.1`** |
...
| **Cubre** | los **50 ficheros `.py`** de `src/`: **31 módulos reales** (**11 920** líneas) + 19 `__init__.py`, ✅ **todos vacíos** |
...
| [10-invariantes-y-trampas.md](10-invariantes-y-trampas.md) | **Lee esto siempre.** **42** trampas con síntoma observable |
...
| **Cualquier cosa de la interfaz** | [06](06-gui-y-vistas.md) → trampas **29-37 y 39-42** (todas de CustomTkinter y de layout) → [11 §1 y §1b](11-playbooks.md) |
...
| **Colocar widgets a mano (`place()`), calcular anchos** | trampas **39, 29 y 30** → [06 §6](06-gui-y-vistas.md) → [09 §6d](09-verificacion-y-pruebas.md) |
| **Que algo se adapte al ancho de la ventana** | trampas **41 y 32** → [06 §6](06-gui-y-vistas.md) (`PosterGrid`) → [09 §6f](09-verificacion-y-pruebas.md) |
| **Pintar texto que viene de un proveedor** | trampa **42** → [06 §8](06-gui-y-vistas.md) → [09 §6g](09-verificacion-y-pruebas.md) |
| **Atar un clic o un hover encima de un widget de CustomTkinter** | trampas **40, 35, 37 y 34** → [06 §6](06-gui-y-vistas.md) → [09 §6b y §6e](09-verificacion-y-pruebas.md) |
| **Abrir la ficha por un episodio concreto** | [03 §12](03-flujos-de-ejecucion.md) → [06 §8](06-gui-y-vistas.md) → trampas **8, 21 y 40** |
